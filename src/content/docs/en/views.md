...
:::info[Why use a CSRF token?]
A CSRF (Cross-Site Request Forgery) token is required when your application uses cookies for authentication. Without it, a malicious website could trick a user's browser into submitting a form to your Jazzy application on their behalf. The hidden `_csrf` field proves that the form was generated by your application. Jazzy automatically injects the `$csrf_token` variable into all templates when `CSRF_ENABLED=true`.
:::
